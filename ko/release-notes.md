...
##### VPC
* [한국/일본/미국 리전] 라우팅테이블의 라우트 생성 시, 게이트웨이에 직접 IP를 입력하는 방식에서 IP를 소유한 장치를 선택하는 방식으로 변경되었습니다. 라우팅테이블에 명시적으로 연결되어 있지 않은 서브넷의 장치도 선택할 수 있습니다.
* [한국/일본/미국 리전] 인터넷게이트웨이 목록에서 IP 정보 대신 연결된 라우팅테이블의 정보를 표시하도록 변경되었습니다. 라우팅테이블의 라우트 탭에서도 연결된 인터넷게이트웨이의 이름이 표시됩니다.
...
* [한국/일본/미국 리전] 로드밸런서와 동일한 VPC에 속한 다른 서브넷의 인스턴스를 로드밸런서의 멤버로 등록할 수 있습니다. 로드밸런서가 속한 서브넷과 인스턴스의 서브넷이 Routing Table에 연결되어야 합니다.
* [한국/일본/미국 리전] 로드밸런서가 속한 VPC가 Peering 연결되어 있다면, Peer VPC에 속한 인스턴스를 로드밸런서의 멤버로 등록할 수 있습니다.  Peer VPC의 Default Routing Table에 연결된 서브넷 상의 인스턴스만 연결 가능합니다.
