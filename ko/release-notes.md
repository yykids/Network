...
* 로드밸런서와 동일한 VPC에 속한 다른 서브넷의 인스턴스를 로드밸런서의 멤버로 등록할 수 있습니다.  로드밸런서가 속한 서브넷과 인스턴스의 서브넷이 Routing Table에 연결되어야 합니다.
* 로드밸런서가 속한 VPC가 Peering 연결된 경우 Peer VPC에 속한 인스턴스를 로드밸런서의 멤버로 등록할 수 있습니다.  Peer VPC의 Default Routing Table에 연결된 서브넷 상의 인스턴스만 연결 가능합니다.
* 로드밸런서에 여러 리스너를 운용하는 경우 모든 리스너에 동일한 멤버 인스턴스를 구성해야 했었는데, 이제는 리스너별 멤버 인스턴스를 서로 다르게 설정할 수 있습니다.
