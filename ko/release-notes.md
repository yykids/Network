...
* 로드밸런서가 속한 VPC가 Peering 연결되어 있다면, Peer VPC에 속한 인스턴스를 로드밸런서의 멤버로 등록할 수 있습니다.  Peer VPC의 Default Routing Table에 연결된 서브넷 상의 인스턴스만 연결 가능합니다.
* 로드밸런서의 개별 리스너에 서로 다른 멤버 인스턴스를 등록할 수 있습니다.
