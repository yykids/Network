...
#### 기능 변경
##### VPC
* [한국/미국 리전] 라우팅테이블의 기본 라우트(VPC 주소 영역 전체로의 로컬 라우트)가 적용되지 않는 문제를 수정했습니다. 기존에는 VPC 내의 서브넷이라도 같은 라우팅 테이블에 연결되어 있는 서브넷 간에만 통신이 가능했지만, 서로 다른 라우팅 테이블에 연결되어 있는 서브넷 간에도 통신이 가능합니다.
* [한국/미국 리전] 피어링 연결 방식을 개선했습니다. 기존에는 VPC의 기본 라우팅 테이블에 연결된 서브넷 간에만 통신이 가능했지만, 기본 라우팅 테이블 뿐 아니라 다른 라우팅 테이블에도 라우트를 추가하여 피어링 통신이 가능하도록 설정할 수 있습니다. 설정 방법은 콘솔 사용 가이드를 참고해 주세요.
