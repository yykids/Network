...
* [한국/일본/미국 리전] 로드밸런서와 동일한 VPC에 속한 다른 서브넷의 인스턴스를 로드밸런서의 멤버로 등>록할 수 있습니다 단, 로드밸런서가 속한 서브넷과 인스턴스의 서브넷이 Routing Table에 연결되어야 합니다.
* [한국/일본/미국 리전] 로드밸런서가 속한 VPC가 Peering 연결되어 있다면, Peer VPC에 속한 인스턴스를 로>드밸런서의 멤버로 등록할 수 있습니다. 단, Peer VPC의 Default Routing Table에 연결된 서브넷 상의 인스턴>스만 연결 가능합니다.
