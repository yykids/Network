...
피어링 생성 시 한 VPC에서 주소범위가 중첩되는 다른 두 VPC와 각각 피어링이 가능한 문제를 수정했습니다. 
* 예를 들어, 다음과 같은 주소 범위를 갖는 세 VPC에 대해, 
	`VPC1(192.168.0.0/16)`, `VPC2(172.16.0.0/16)`, `VPC3(172.16.0.0/12)`
	VPC1과 VPC2를 피어링 한 상태에서 VPC1과 VPC3을 피어링 할 수 없습니다. 이러한 피어링을 허용할 경우, VPC2와 VPC3의 주소범위가 중첩되기 때문에 VPC1에 위치한 인스턴스에서 VPC2(172.16.0.0/16) 대역으로 패킷을 발신하면 VPC2가 아닌 VPC3으로 라우팅 될 수 있습니다.
...
서브넷, 라우팅테이블, 라우트에 대해 최대 생성 가능 개수를 조정했습니다.
각 리소스별 최대 생성 가능 개수는 항목별 생성창의 우측 설명 부분에서 확인하실 수 있습니다.
* 서브넷: VPC당 10개까지 생성이 가능합니다.
* 라우팅테이블: VPC당 10개까지 생성이 가능합니다.
